...
import cn.cqut.lgqs.admin.util.Permission;
import cn.cqut.lgqs.admin.util.PermissionUtil;
import cn.cqut.lgqs.admin.vo.PermVo;
...
import cn.cqut.lgqs.db.service.LgqsPermissionService;
import cn.cqut.lgqs.db.service.LgqsRoleService;
...
import static cn.cqut.lgqs.admin.util.AdminResponseCode.ROLE_NAME_EXIST;
import static cn.cqut.lgqs.admin.util.AdminResponseCode.ROLE_USER_EXIST;
...
public class AdminRoleController {
    ...
    @Autowired
    private LgqsRoleService roleService;
    @Autowired
    private LgqsPermissionService permissionService;
    @Autowired
    ...
    public Object list(String name,
        ...
        List<LgqsRole> roleList = roleService.querySelective(name, page, limit, sort, order);
        return ResponseUtil.okList(roleList);
    }

    @GetMapping("/options")
    public Object options() {
        List<LgqsRole> roleList = roleService.queryAll();
    ...
    public Object read(@NotNull Integer id) {
        LgqsRole role = roleService.findById(id);
        return ResponseUtil.ok(role);
    }


    private Object validate(LgqsRole role) {
    ...
    @PostMapping("/create")
    public Object create(@RequestBody LgqsRole role) {
    ...
    @PostMapping("/update")
    public Object update(@RequestBody LgqsRole role) {
        Object error = validate(role);
        if (error != null) {
            return error;
        }
    ...
    @PostMapping("/delete")
    public Object delete(@RequestBody LgqsRole role) {
        ...
        List<LgqsAdmin> adminList = adminService.all();
        for (LgqsAdmin admin : adminList) {
    ...
    private List<PermVo> getSystemPermissions() {
        final String basicPackage = "org.cqut.lgqs.admin";
    ...
    public Object getPermissions(Integer roleId) {
        ...
        LgqsAdmin currentAdmin = (LgqsAdmin) currentUser.getPrincipal();
    ...
    public Object updatePermissions(@RequestBody String body) {
        ...
        for (String permission : permissions) {
            LgqsPermission lgqsPermission = new LgqsPermission();
            lgqsPermission.setRoleId(roleId);
            lgqsPermission.setPermission(permission);
            permissionService.add(lgqsPermission);
        }
        return ResponseUtil.ok();
    }
