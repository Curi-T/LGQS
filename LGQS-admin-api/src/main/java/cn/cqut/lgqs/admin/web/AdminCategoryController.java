...
import cn.cqut.lgqs.admin.vo.CategoryVo;
import cn.cqut.lgqs.core.util.ResponseUtil;
import cn.cqut.lgqs.db.domain.LgqsCategory;
import cn.cqut.lgqs.db.service.LgqsCategoryService;
...
public class AdminCategoryController {
    ...
    @RequiresPermissionsDesc(menu = {"餐厅管理", "类目管理"}, button = "查询")
    @GetMapping("/list")
    public Object list() {
        List<CategoryVo> categoryVoList = new ArrayList<>();

        List<LgqsCategory> categoryList = categoryService.queryByPid(0);
        for (LgqsCategory category : categoryList) {
            ...
            List<LgqsCategory> subCategoryList = categoryService.queryByPid(category.getId());
            for (LgqsCategory subCategory : subCategoryList) {
    ...
    private Object validate(LgqsCategory category) {
    ...
    @RequiresPermissionsDesc(menu = {"餐厅管理", "类目管理"}, button = "添加")
    @PostMapping("/create")
    public Object create(@RequestBody LgqsCategory category) {
    ...
    @RequiresPermissionsDesc(menu = {"餐厅管理", "类目管理"}, button = "详情")
    @GetMapping("/read")
    public Object read(@NotNull Integer id) {
        LgqsCategory category = categoryService.findById(id);
        return ResponseUtil.ok(category);
    }

    @RequiresPermissions("admin:category:update")
    @RequiresPermissionsDesc(menu = {"餐厅管理", "类目管理"}, button = "编辑")
    @PostMapping("/update")
    public Object update(@RequestBody LgqsCategory category) {
        Object error = validate(category);
        if (error != null) {
            return error;
        }
    ...
    @RequiresPermissionsDesc(menu = {"餐厅管理", "类目管理"}, button = "删除")
    @PostMapping("/delete")
    public Object delete(@RequestBody LgqsCategory category) {
    ...
    public Object catL1() {
        ...
        for (LgqsCategory category : l1CatList) {
