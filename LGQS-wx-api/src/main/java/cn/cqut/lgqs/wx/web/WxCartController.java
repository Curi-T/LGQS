...
import static cn.cqut.lgqs.wx.util.WxResponseCode.GOODS_NO_STOCK;
import static cn.cqut.lgqs.wx.util.WxResponseCode.GOODS_UNSHELVE;
...
public class WxCartController {
    ...
    public Object index(@LoginUser Integer userId) {
        ...
        List<LgqsCart> list = cartService.queryByUid(userId);
        List<LgqsCart> cartList = new ArrayList<>();
        // TODO
        // 如果系统检查餐品已删除或已下架，则系统自动删除。
        // 更好的效果应该update是告知用户餐品失效，允许用户点击按钮来清除失效餐品。
        for (LgqsCart cart : list) {
            LgqsGoods goods = goodsService.findById(cart.getGoodsId());
            if (goods == null || !goods.getIsOnSale()) {
                cartService.deleteById(cart.getId());
                logger.debug("系统自动删除失效点餐车餐品 goodsId=" + cart.getGoodsId() + " productId=" + cart.getProductId());
            } else {
                cartList.add(cart);
            }
        }
     ...
     *
     * @param userId 用户ID
     * @param cart   点餐车餐品信息， { goodsId: xxx, productId: xxx, number: xxx }
     * @return 加入点餐车操作结果
     */
    @PostMapping("add")
    public Object add(@LoginUser Integer userId, @RequestBody LgqsCart cart) {
        if (userId == null) {
            return ResponseUtil.unlogin();
        }
        if (cart == null) {
        ...
        //判断餐品是否可以购买
        LgqsGoods goods = goodsService.findById(goodsId);
        if (goods == null || !goods.getIsOnSale()) {
            return ResponseUtil.fail(GOODS_UNSHELVE, "餐品已下架");
        }

        LgqsGoodsProduct product = productService.findById(productId);
        //判断点餐车中是否存在此规格餐品
        LgqsCart existCart = cartService.queryExist(goodsId, productId, userId);
        if (existCart == null) {
            //取得规格的信息,判断规格库存
     ...
     * <p>
     * 和add方法的区别在于：
     * 1. 如果点餐车内已经存在点餐车餐品，前者的逻辑是数量添加，这里的逻辑是数量覆盖
     * 2. 添加成功以后，前者的逻辑是返回当前点餐车餐品数量，这里的逻辑是返回对应点餐车项的ID
     *
     * @param userId 用户ID
     * @param cart   点餐车餐品信息， { goodsId: xxx, productId: xxx, number: xxx }
     * @return 立即购买操作结果
     */
    @PostMapping("fastadd")
    public Object fastadd(@LoginUser Integer userId, @RequestBody LgqsCart cart) {
        if (userId == null) {
            return ResponseUtil.unlogin();
        }
        if (cart == null) {
     ...
     *
     * @param userId 用户ID
     * @param cart   点餐车餐品信息， { id: xxx, goodsId: xxx, productId: xxx, number: xxx }
     * @return 修改结果
     */
    @PostMapping("update")
    public Object update(@LoginUser Integer userId, @RequestBody LgqsCart cart) {
        if (userId == null) {
            return ResponseUtil.unlogin();
        }
        Integer productId = cart.getProductId();
        ...
        //判断是否存在该订单
        // 如果不存在，直接返回错误
        LgqsCart existCart = cartService.findById(userId, id);
        if (existCart == null) {
     ...
     * 点餐车餐品餐品勾选状态
     * <p>
     * 如果原来没有勾选，则设置勾选状态；如果餐品已经勾选，则设置非勾选状态。
     *
     * @param userId 用户ID
     * @param body   点餐车餐品信息， { productIds: xxx, isChecked: 1/0 }
     * @return 点餐车信息
     */
    @PostMapping("checked")
     ...
     *
     * @param userId 用户ID
     * @param body   点餐车餐品信息， { productIds: xxx }
     * @return 点餐车信息
     * 成功则
     * {
     * errno: 0,
    ...
    public Object goodscount(@LoginUser Integer userId) {
        ...
        int goodsCount = 0;
        List<LgqsCart> cartList = cartService.queryByUid(userId);
        for (LgqsCart cart : cartList) {
            goodsCount += cart.getNumber();
        }
     ...
     *
     * @param userId    用户ID
     * @param cartId    点餐车餐品ID：
     *                  如果点餐车餐品ID是空，则下单当前用户所有点餐车餐品；
     *                  如果点餐车餐品ID非空，则只下单当前点餐车餐品。
    ...
    public Object checkout(@LoginUser Integer userId, Integer cartId, Integer addressId, Integer couponId, Integer userCouponId, Integer grouponRulesId) {
        ...
        // 收货地址
        LgqsAddress checkedAddress = null;
        ...
        if (checkedAddress == null) {
            ...
            if (checkedAddress == null) {
                checkedAddress = new LgqsAddress();
        ...
        for (LgqsCouponUser couponUser : couponUserList) {
            LgqsCoupon coupon = couponVerifyService.checkCoupon(userId, couponUser.getCouponId(), couponUser.getId(), checkedGoodsPrice, checkedGoodsList);
            if (coupon == null) {
                continue;
            }
