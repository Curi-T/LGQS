...
import cn.cqut.lgqs.db.service.LgqsSearchHistoryService;
...
public class WxSearchController {
    ...
    @Autowired
    private LgqsKeywordService keywordsService;
    @Autowired
    ...
    public Object index(@LoginUser Integer userId) {
        //取出输入框默认的关键词
        LgqsKeyword defaultKeyword = keywordsService.queryDefault();
        //取出热闹关键词
        List<LgqsKeyword> hotKeywordList = keywordsService.queryHots();

        List<LgqsSearchHistory> historyList = null;
        if (userId != null) {
            //取出用户历史关键字
    ...
    public Object helper(@NotEmpty String keyword,
        ...
        List<LgqsKeyword> keywordsList = keywordsService.queryByKeyword(keyword, page, limit);
        String[] keys = new String[keywordsList.size()];
        int index = 0;
        for (LgqsKeyword key : keywordsList) {
