...
import cn.cqut.lgqs.db.service.LgqsAddressService;
import cn.cqut.lgqs.db.service.LgqsRegionService;
import cn.cqut.lgqs.wx.annotation.LoginUser;
import cn.cqut.lgqs.wx.service.GetRegionService;
...
public class WxAddressController extends GetRegionService {
	...
	public Object list(@LoginUser Integer userId) {
		...
		List<LgqsAddress> addressList = addressService.queryByUid(userId);
	...
	public Object detail(@LoginUser Integer userId, @NotNull Integer id) {
		...
		LgqsAddress address = addressService.query(userId, id);
		if (address == null) {
	...
	private Object validate(LgqsAddress address) {
	 ...
	 */
	@PostMapping("save")
	public Object save(@LoginUser Integer userId, @RequestBody LgqsAddress address) {
		if (userId == null) {
			return ResponseUtil.unlogin();
		}
		...
		if (address.getId() == null || address.getId().equals(0)) {
			...
			LgqsAddress lgqsAddress = addressService.query(userId, address.getId());
			if (lgqsAddress == null) {
				return ResponseUtil.badArgumentValue();
			}
	 ...
	 */
	@PostMapping("delete")
	public Object delete(@LoginUser Integer userId, @RequestBody LgqsAddress address) {
		if (userId == null) {
			return ResponseUtil.unlogin();
		}
		...
		LgqsAddress lgqsAddress = addressService.query(userId, id);
		if (lgqsAddress == null) {
			return ResponseUtil.badArgumentValue();
		}
