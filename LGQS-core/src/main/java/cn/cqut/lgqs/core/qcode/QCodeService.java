package cn.cqut.lgqs.core.qcode;

import cn.binarywang.wx.miniapp.api.WxMaService;
import cn.cqut.lgqs.core.storage.StorageService;
...
public class QCodeService {
    ...
    public String createGrouponShareImage(String goodName, String goodPicUrl, LgqsGroupon groupon) {
        try {
            //创建该餐品的二维码
            File file = wxMaService.getQrcodeService().createWxaCodeUnlimit("groupon," + groupon.getId(), "pages" +
                    "/index/index");
            FileInputStream inputStream = new FileInputStream(file);
            //将餐品图片，餐品名字,餐厅名字画到模版图中
            ...
            //存储分享图
            LgqsStorage storageInfo = storageService.store(inputStream2, imageData.length, "image/jpeg",
    ...
    public String createGoodShareImage(String goodId, String goodPicUrl, String goodName) {
        ...
        try {
            ...
            ByteArrayInputStream inputStream2 = new ByteArrayInputStream(imageData);
            //存储分享图
            LgqsStorage lgqsStorage = storageService.store(inputStream2, imageData.length, "image/jpeg",
                    getKeyName(goodId));

            return lgqsStorage.getUrl();
        } catch (WxErrorException e) {
            logger.error(e.getMessage(), e);
        } catch (FileNotFoundException e) {
            logger.error(e.getMessage(), e);
